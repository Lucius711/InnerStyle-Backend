...
public interface MeshyTaskThumbnailRepository extends JpaRepository<MeshyTaskThumbnail, UUID> {
    ...
    @Query(value = "SELECT task_id AS taskId FROM dtb_meshy_task_thumbnail"
    ...
    @Query(value = "SELECT data AS data FROM dtb_meshy_task_thumbnail WHERE task_id = :taskId", nativeQuery = true)
    LegacyBlobData findLegacyData(@Param("taskId") UUID taskId);

    @Modifying
    @Query(value = "UPDATE dtb_meshy_task_thumbnail SET storage_key = :key, data = NULL"
